...
func NewClient(ctx context.Context, connection string, username string, password string) (Client, error) {
	credential := options.Credential{
		Username:      username,
		Password:      password,
		AuthMechanism: "SCRAM-SHA-256",
	}
	c, err := mongo.Connect(ctx, options.Client().ApplyURI(connection).SetAuth(credential))
